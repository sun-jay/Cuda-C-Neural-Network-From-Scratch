...
class CudaTimer {
    public: CudaTimer(const std::string & name): name_(name) {
...
public:
    ...
    float calc_loss_cpu(Matrix y_pred, Matrix y_true){
        Matrix y_pred_clipped(y_pred.rows, y_pred.cols);
        ...
        Matrix correct_confidences(1,y_true.cols);
    ...
    void backward(Matrix& dvalues, Matrix& y_true) {
        ...
        int totalLen = y_true.cols;  // Number of samples in the batch
        int blockSize = 1024;
        int numBlocks = (totalLen + blockSize - 1) / blockSize;
